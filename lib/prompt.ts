...
imageUrl 为匹配物种的展示图片 URL（可用 Wikimedia Commons 等免版权图库）。
matchReason 为免费版内容（2-4 句短文，解释匹配理由，须为原创推理）。
